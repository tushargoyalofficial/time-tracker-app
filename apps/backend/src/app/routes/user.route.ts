import { IAddUser, ILoginUser } from '@time-tracker-app/models';
...
router.post('/login', async (req: Request, res: Response) => {
  ...
  if (data && Object.keys(data).length !== 0 && data.email && data.password ) {
    ...
    const err = {
      status: 301,
      success: false,
      message: 'Invalid or incomplete data. Required data are email and password',
    };
    return res.status(301).send(err);
  }
});
